...
(function(){
  ...
  // mars 2ten added
  theme.BlogPostsSlider = (function() {
    var defaults = {
      adaptiveHeight: true,
      avoidReflow: true,
      pageDots: false,
    ...
    function BlogPostsSlider(container) {
      this.container = container;
      this.timeout;
      var sectionId = container.getAttribute('data-section-id');
      this.slideshow = container.querySelector('#BlogPostsSlider-' + sectionId);
      this.namespace = '.blog-posts-slider-' + sectionId;
    ...
    BlogPostsSlider.prototype = Object.assign({}, BlogPostsSlider.prototype, {
      init: function() {
        // Do not wrap when only a few blocks
        if (this.slideshow.dataset.count <= 3) {
          defaults.wrapAround = false;
      ...
      onBlockSelect: function(evt) {
        var slide = this.slideshow.querySelector('.blog-post-slide--' + evt.detail.blockId)
        var index = parseInt(slide.dataset.index);
  ...
  theme.LogoSlider = (function() {
    ...
    function LogoSlider(container) {
      ...
      this.timeout;
      var sectionId = container.getAttribute('data-section-id');
      this.slideshow = container.querySelector('#LogoSlider-' + sectionId);
      this.namespace = '.logo-slider-' + sectionId;
    ...
    LogoSlider.prototype = Object.assign({}, LogoSlider.prototype, {
      init: function() {
        // Do not wrap when only a few blocks
        if (this.slideshow.dataset.count <= 5) {
          defaults.wrapAround = false;
        }
        ...
        var logoCaptionHolder = document.querySelector('.logo-caption--holder');
        function handleMouseOver(event) {
          var captionElement = event.querySelector('.logo-slide--caption');
          var parent = captionElement.closest('.logo-slider');
          logoCaptionHolder = parent.querySelector('.logo-caption--holder');
          if (captionElement) {
            logoCaptionHolder.style.opacity = 0;
            var content = captionElement.innerHTML;
            setTimeout(function(){
              logoCaptionHolder.innerHTML = content;
              logoCaptionHolder.style.opacity = 1;         
            },500);
          }
        }
        ...
        const logoSlideElements = document.querySelectorAll('.logo-slide');

        logoSlideElements.forEach((element) => {
          element.addEventListener('mouseover', () => handleMouseOver(element));
          element.addEventListener('mouseout', handleMouseOut);
        });
       
        var flickerDiv = document.querySelector('.logo-slider-section .logo-caption--holder');
    
        if (flickerDiv) {
          const interval = setInterval(() => {
            const flickDiv = document.querySelector('.flickity-page-dots');
            if (flickDiv) {
              const logoSliderSections = document.querySelectorAll('.logo-slider-section');
              
              logoSliderSections.forEach(function (logoSliderSection) {
                const flickityPageDots = logoSliderSection.querySelector('.flickity-page-dots');
                const logoCaptionHolder = logoSliderSection.querySelector('.logo-caption--holder');
              
                if (flickityPageDots && logoCaptionHolder) {
                  flickityPageDots.parentNode.insertBefore(logoCaptionHolder, flickityPageDots);
                }
              });
              clearInterval(interval);
              var carousel = document.querySelector('.logo-slider')
              var flick = Flickity.data( carousel )
              //flick.on( 'select', function(index){ console.log(flick.slides.length)} );
              flick.on( 'change', function( index ) {
                  //var slides = flick.slides.length;
                  //var cells = flick.cells.length;
                  //var cells_per_slide = Math.floor( cells/slides );
                  var cells_per_slide = 5;
                  var selected_cell_index = ( index * cells_per_slide );
                  var cellElements = flick.getCellElements();
                  var captionElement = cellElements[selected_cell_index].querySelector('.logo-slide--caption');
                  var parent = captionElement.closest('.logo-slider');
                  var logoCaptionHolder = parent.querySelector('.logo-caption--holder');
                  if (captionElement) {
                    logoCaptionHolder.style.opacity = 0;
                    var content = captionElement.innerHTML;
                    setTimeout(function(){
      ...
      onBlockSelect: function(evt) {
        var slide = this.slideshow.querySelector('.logo-slide--' + evt.detail.blockId)
        var index = parseInt(slide.dataset.index);
  ...
  DOMready(function(){
    ...
    // mars 2ten added - see function above
    theme.sections.register('blog-posts-slider', theme.BlogPostsSlider);
    theme.sections.register('logo-slider', theme.LogoSlider);
    // end mars 2ten added
